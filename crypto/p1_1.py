...
def letter_position(s):
    ...
    for idx in range(len(s)):
        if (s[idx] >= 'A' and s[idx] <= 'Z') or (s[idx] >= 'a'
                                                 and s[idx] <= 'z'):
            position.append(idx)
    return position
...
def find_space(cipher):
    ...
    for cipher_idx_1 in range(NUM_CIPHER):
        ...
        for cipher_idx_2 in range(NUM_CIPHER):
            plain_xor = strxor(cipher[cipher_idx_1].decode('hex'),
                               cipher[cipher_idx_2].decode('hex'))
            if cipher_idx_2 != cipher_idx_1:
                space_xor.append(letter_position(
                    plain_xor))  # record the possible value of space
...
def calculate_key(cipher):
    ...
    for cipher_idx_1 in range(NUM_CIPHER):
        for position in range(len(space[cipher_idx_1])):
            idx = space[cipher_idx_1][
                position] * 2  # ciphertext is hex-encoded, so its scale times two
            key[space[cipher_idx_1][position]] = ord(
                (ciphertexts[cipher_idx_1][idx] +
                 ciphertexts[cipher_idx_1][idx + 1]).decode('hex')) ^ ord(
                     ' ')  # derive key
    key_str = ""
    for k in key:
        key_str += chr(k)
...
if __name__ == "__main__":
    ...
    print(key)
    for letter in strxor(ciphertexts[10].decode('hex'),
                         key):  # decrypt the target cipher
